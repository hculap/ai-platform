...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <section 
      id="hero" 
      data-section 
      className="relative flex min-h-screen flex-col justify-center px-4 pt-28 pb-16 sm:px-6 md:pt-32 lg:px-8 lg:pt-36"
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center text-center">
        ...
        <div className={`transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          <h1 className="mb-6 text-3xl font-extrabold leading-tight sm:text-5xl md:text-6xl lg:text-7xl">
            <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent">
              AI Business Growth Toolkit
            </span>
          </h1>

          <p className="mb-6 text-lg font-medium text-gray-900 sm:text-2xl md:text-3xl lg:text-4xl">
        ...
        >
          <p className="mx-auto mb-10 max-w-3xl text-base font-medium text-gray-900 sm:mb-12 sm:text-xl md:text-2xl lg:text-3xl">
        ...
        {/* URL Input Form */}
        <div 
          className={`mx-auto mb-12 w-full max-w-3xl transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
          ...
          <form onSubmit={handleSubmit} className="relative group">
            <div className="relative">
              <input 
                ...
                className={`w-full rounded-2xl border-2 bg-white/90 px-5 py-4 pr-20 text-base backdrop-blur-sm transition-all duration-300 focus:outline-none shadow-lg hover:shadow-xl group-hover:scale-[1.02] sm:px-8 sm:py-6 sm:text-lg ${
                  url.length === 0 
            ...
            <div className="mt-8 space-y-4">
              <button 
                ...
                className={`w-full rounded-2xl px-8 py-4 text-base font-semibold transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-blue-500/25 disabled:cursor-not-allowed disabled:opacity-50 disabled:transform-none sm:w-auto sm:px-12 sm:text-lg ${
                  isValid && !isAnalyzing
                    ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-2xl hover:from-blue-700 hover:to-purple-700 hover:shadow-purple-500/25' 
              ...
              {/* Alternative CTA */}
              <div className="mt-6 flex flex-col items-stretch gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
                <span className="text-center text-sm text-gray-500 sm:text-left">lub</span>
                <button
                  type="button"
                  onClick={onSkipToForm}
                  className="rounded-xl border border-blue-200 bg-white/80 px-6 py-3 text-sm font-semibold text-blue-600 transition-all duration-300 hover:scale-105 hover:bg-white hover:shadow-lg sm:text-base"
                >
        ...
        {/* Key benefits */}
        <div 
          className={`mx-auto grid w-full max-w-4xl grid-cols-1 gap-6 transition-all duration-1000 transform sm:grid-cols-2 sm:gap-8 lg:grid-cols-4 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
